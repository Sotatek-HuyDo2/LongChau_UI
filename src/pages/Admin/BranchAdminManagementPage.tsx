...
const BranchAdminManagementPage = () => {
  ...
  const [dataSearch, setDataSearch] = useState<IAdminBranch[]>(
    MOCK_CATEGORY_MEDICINE,
  );

  const dataRef = useRef<IAdminBranch[]>([]);

  const handleSearch = () => {
    let dataFilter = dataRef.current;

    if (valueSearch) {
      dataFilter = dataFilter.filter((item: IAdminBranch) =>
  ...
  const _renderContentTable = (data: IAdminBranch[]) => {
    return (
      <Box>
        {dataSearch.map((data: IAdminBranch, id: number) => {
          return (
    ...
    <BasePage>
      <Box className="category" w="full">
        <Flex
          ...
          color={'#2167df'}
        >
          Quản lý branch Admin
        </Flex>
        <Box className={'category__search'}>
          <Flex alignItems={'center'}>
            <Box className={'category__search-title'}>Branch Admin:</Box>
